...
На события таймера может подписываться любой класс, реализующий интерфейс ICountDownNotifier. В метод Init передаются методы, которые должны быть выполнены при наступлении событий Started, Tick и Stopped таймера. Предусмотреть случай, когда вместо делегата передается null - это не должно повлиять на работу таймера. Метод Run запускает таймер.
...
5. Классы event args необходимо разместить в папке EventArgs.
6. Вносить изменения в unit тесты запрещено.
7. Необходимо добиться успешного прохождения всех unit тестов.
